'''
Adaptation of Ladder Network ("Semi-Supervised Learning with Ladder Networks" by A. Rasmus et al.) to our problem
'''
...
print("===  Loading Data ===")
train_labeled = pd.read_hdf("train_labeled.h5", "train")  # 9000
...
X_train_labeled, y_train_labeled = shuffle(X_train_labeled, y_train_labeled, random_state=1234)
X_train, X_valid = np.split(X_train_labeled, [int(0.9 * X_train_labeled.shape[0])], axis=0)
y_train, y_valid = np.split(y_train_labeled, [int(0.9 * y_train_labeled.shape[0])], axis=0)
y_train = keras.utils.to_categorical(y_train, 10)
y_valid = keras.utils.to_categorical(y_valid, 10)

train_unlabeled = pd.read_hdf("train_unlabeled.h5", "train")  # 21000
X_train_unlabeled = train_unlabeled.values

test = pd.read_hdf("test.h5", "test")  # 8000
X_test = test.values
index_out = test.index.values

# scaling the data
scaler = preprocessing.StandardScaler().fit(np.concatenate((X_train_unlabeled, X_train), axis=0))
...
print("=== Training ===")
print("Initial Accuracy: ", sess.run(accuracy, feed_dict={inputs: X_valid, outputs: y_valid, training: False}), "%")

combined = list(zip(X_train, y_train))

for i in tqdm(range(int(i_iter), int(num_iter))):
    batch = random.sample(combined, batch_size)
    labeled_images, labels = zip(*batch)
    unlabeled_images = random.sample(list(X_train_unlabeled), batch_size)
    ...
    if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
        ...
        print("Epoch ", epoch_n, ", Accuracy: ", sess.run(accuracy, feed_dict={inputs: X_valid, outputs: y_valid, training: False}), "%")
        with open('train_log', 'a') as train_log:
            # write test accuracy to file "train_log"
            train_log_w = csv.writer(train_log)
            log_i = [epoch_n] + sess.run([accuracy], feed_dict={inputs: X_valid, outputs: y_valid, training: False})
            train_log_w.writerow(log_i)

print("Final Accuracy: ", sess.run(accuracy, feed_dict={inputs: X_valid, outputs: y_valid, training: False}), "%")

print("=== Testing ===")
print("Validation Accuracy: ", sess.run(accuracy, feed_dict={inputs: X_valid, outputs: y_valid, training: False}), "%")

preds = sess.run(tf.argmax(y, 1), feed_dict={inputs: X_test, training: False})
data = {'Id' : index_out.astype(int), 'y' : preds}
df = pd.DataFrame(data)
df.to_csv('ladder_output.csv', index=False)
